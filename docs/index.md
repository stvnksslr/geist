...
**geist** is a GPU-accelerated terminal emulator for **Windows**, written in
...
This site documents the geist application and — most importantly — the
**touch points** between the three systems it stitches together:

```mermaid
flowchart LR
    OS["Operating System<br/>(Windows)"]
    geist["geist<br/>(this application)"]
    VT["libghostty-vt<br/>(Ghostty's VT engine)"]

    OS <-->|"ConPTY · clipboard · GPU<br/>window · keyboard · mouse"| geist
    geist <-->|"bytes in · grid out<br/>key/mouse/paste encode"| VT
...
- **[OS / libghostty touch points](touchpoints/overview.md)** — every place
  geist crosses a boundary into Windows or into libghostty-vt.
...
| Backend-neutral terminal contract | geist | `engine/mod.rs` |
| Shell process + I/O | ConPTY via portable-pty | `pty.rs` |
| Per-session glue (selection, OSC 52) | geist | `session.rs` |
| Glyph atlas + instanced-quad renderer | geist + wgpu | `render/mod.rs`, `render/atlas.rs` |
| Config, shell profiles, clipboard | geist + Windows | `config.rs`, `profiles.rs`, `clipboard.rs` |
...
See the [roadmap in the README](https://github.com/stvnksslr/geist#roadmap-toward-macos-ghostty-parity)
