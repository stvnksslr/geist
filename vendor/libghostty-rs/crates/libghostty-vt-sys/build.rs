...
fn build_vendored(link_mode: LinkMode) {
    ...
    match link_mode {
        ...
        LinkMode::Static => {
            // On Windows/MSVC the *static* archive is `ghostty-vt-static.lib`,
            // while `ghostty-vt.lib` is the *import library* for the DLL. The
            // upstream `static=ghostty-vt` resolves to the import lib, which
            // makes the binary depend on `ghostty-vt.dll` at runtime (and that
            // dynamic path crashes on Windows). Link the real static archive.
            if target.contains("windows") {
