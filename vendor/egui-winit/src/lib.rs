...
pub fn create_winit_window_attributes(
    ...
    // its target with `CreateTargetForHwnd(hwnd, topmost = false)` (wgpu-hal
    // `dx12/dcomp.rs`), which places the composition visual *below* the window's
    // redirection surface. Without this flag the window still has one, so the
    // per-pixel-alpha content composites over that opaque surface instead of over
    // the desktop: the window renders as an opaque grey wash and
    // `background-opacity` has no visible effect. The style is honored only at
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        window_attributes =
            window_attributes.with_no_redirection_bitmap(transparent.unwrap_or(false));
    }
